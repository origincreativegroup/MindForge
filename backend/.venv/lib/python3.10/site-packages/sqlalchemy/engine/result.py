...
class ResultMetaData:
    ...
    def _index_for_key(self, key: _KeyIndexType, raiseerr: bool) -> int | None:
        raise NotImplementedError()
    ...
    def __init__(
        ...
        if extra:
            recs_names = [
                ...
                for index, (name, extras) in enumerate(
                    zip(self._keys, extra, strict=False)
                )
            ]
        else:
            recs_names = [
...
class ResultInternal(InPlaceGenerative, Generic[_R]):
    ...
    def _manyrow_getter(self) -> Callable[..., list[_R]]:
        ...
        if self._unique_filter_state:
            ...
            def manyrows(self: ResultInternal[_R], num: int | None) -> list[_R]:
                collect: list[_R] = []
...
class ChunkedIteratorResult(IteratorResult[_TP]):
    ...
    def __init__(
        self,
        cursor_metadata: ResultMetaData,
        chunks: Callable[[int | None], Iterator[Sequence[_InterimRowType[_R]]]],
