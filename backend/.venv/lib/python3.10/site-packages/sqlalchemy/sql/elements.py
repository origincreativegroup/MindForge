...
class ClauseElement(
    ...
    def _compile_w_cache(
        ...
        **kw: Any,
    ) -> tuple[Compiled, Sequence[BindParameter[Any]] | None, CacheStats]:
        ...
        if type_ is None:
            ...
            if value:
                if expanding:
                    ...
                    check_value = value
                cast(
                    "BindParameter[tuple[Any, ...]]", self
                ).type = type_._resolve_values_to_types(check_value)
            else:
                cast("BindParameter[tuple[Any, ...]]", self).type = type_
        else:
            self.type = type_
...
class TextClause(
    ...
    def self_group(self, against: OperatorType | None = None) -> Self | Grouping[Any]:
...
class True_(SingletonConstant, roles.ConstExprRole[bool], ColumnElement[bool]):
    ...
    @classmethod
    def _ifnone(cls, other: ColumnElement[Any] | None) -> ColumnElement[Any]:
        if other is None:
...
class OperatorExpression(ColumnElement[_T]):
    ...
    def self_group(self, against: OperatorType | None = None) -> Self | Grouping[_T]:
        if (
            self.group
...
class BooleanClauseList(ExpressionClauseList[bool]):
    ...
    def self_group(self, against: OperatorType | None = None) -> Self | Grouping[bool]:
        if not self.clauses:
            return self
        else:
...
class TypeCoerce(WrapsColumnExpression[_T]):
    ...
    def self_group(self, against: OperatorType | None = None) -> TypeCoerce[_T]:
...
class Grouping(GroupedElement, ColumnElement[_T]):
    ...
    def __init__(self, element: TextClause | ClauseList | ColumnElement[_T]):
        self.element = element
...
class FunctionFilter(Generative, ColumnElement[_T]):
    ...
    def over(
        self,
        partition_by: Iterable[_ColumnExpressionArgument[Any]]
        | _ColumnExpressionArgument[Any]
        | None = None,
        order_by: Iterable[_ColumnExpressionArgument[Any]]
        | _ColumnExpressionArgument[Any]
        | None = None,
        range_: tuple[int | None, int | None] | None = None,
        rows: tuple[int | None, int | None] | None = None,
...
class NamedColumn(KeyedColumnElement[_T]):
    ...
    def _gen_tq_label(self, name: str, dedupe_on_key: bool = True) -> str | None:
        return name
