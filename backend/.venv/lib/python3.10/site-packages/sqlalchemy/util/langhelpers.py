...
def clsname_as_plain_name(cls: type[Any], use_name: str | None = None) -> str:
...
def monkeypatch_proxied_specials(
    ...
    for method in dunders:
        ...
        py = (
            "def %(method)s%(fn_args)s: return %(name)s.%(method)s%(d_args)s" % locals()
        )
...
def add_parameter_text(params: Any, text: str) -> Callable[[_F], _F]:
    ...
    def decorate(fn):
        ...
        if doc:
            doc = inject_param_text(doc, dict.fromkeys(params, text))
        fn.__doc__ = doc
        return fn
