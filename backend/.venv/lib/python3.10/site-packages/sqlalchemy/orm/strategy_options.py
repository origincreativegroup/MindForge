...
class _AbstractLoad(traversals.GenerativeOnTraversal, LoaderOption):
    ...
    def _coerce_strat(self, strategy: _StrategySpec | None) -> _StrategyKey | None:
            ...
            if len(list(entities)) != 1:
                if raiseerr:
                    raise sa_exc.ArgumentError(
                        ...
                        f"""{
                            ", ".join(
                                f"Load({ent}).some_option('*')" for ent in entities
                            )
                        }."""
                    )
    ...
    def _raise_for_no_match(self, parent_loader, mapper_entities):
        ...
        if not found_entities:
            ...
            raise sa_exc.ArgumentError(
                ...
                f"""{
                    ", ".join(
                        str(x.entity_zero) for x in mapper_entities if x.entity_zero
                    )
                }. Please """
...
def _raise_for_does_not_link(path, attrname, parent_entity):
    if len(path) > 1:
        ...
        raise sa_exc.ArgumentError(
            f'ORM mapped attribute "{attrname}" does not link mapped class "{path[-1]}"'
        )
