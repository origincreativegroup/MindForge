...
def extract_process(texts: list[str]) -> dict[str, list[str]]:
    ...
    try:
        ...
        if parsed and any(parsed.values()):
            out = {
                "steps": [
                    str(x).strip() for x in parsed.get("steps", []) if str(x).strip()
                ][:25],
                "actors": [
                    str(x).strip() for x in parsed.get("actors", []) if str(x).strip()
                ][:25],
                "tools": [
                    str(x).strip() for x in parsed.get("tools", []) if str(x).strip()
                ][:25],
                "decisions": [
                    str(x).strip()
