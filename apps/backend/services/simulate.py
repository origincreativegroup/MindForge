...
def simulate(
    process: dict[str, list[str]], recent_emotions: list[str], scale: float = 1.0
) -> dict[str, any]:
    steps = process.get("steps", []) or []
    # compute frustration bias from recent emotions
    fbias = (
        0.3
        if any(e in ("frustrated", "concerned") for e in recent_emotions[-5:])
        else 0.0
    )
    ...
    summary = (
        "Not enough structure to simulate."
        if not steps
        else f"Projected bottleneck under scale x{scale:.1f}: step {top[0] + 1} — “{top[1]}”"
    )
    return {
        "scale": scale,
