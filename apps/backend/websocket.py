...
async def websocket_process_endpoint(websocket: WebSocket, process_id: str = "default"):
    ...
    try:
        ...
        await manager.send_personal_message(
            json.dumps(
                {"type": "connection", "status": "connected", "process_id": process_id}
            ),
            websocket,
        )
...
async def handle_websocket_message(
    message: dict, websocket: WebSocket, process_id: str
):
    ...
    if msg_type == "ping":
        ...
        await manager.broadcast_to_process(
            json.dumps({"type": "annotation", "payload": annotation, "sender": "user"}),
            process_id,
        )
        ...
        await manager.broadcast_to_process(
            json.dumps({"type": "update", "payload": process_data}), process_id
        )
        ...
        await manager.broadcast_to_process(
            json.dumps({"type": "cursor", "payload": position, "sender": websocket}),
            process_id,
        )
...
async def broadcast_annotation(annotation: dict, process_id: str = "default"):
    ...
    """
    message = json.dumps(
        {"type": "annotation", "payload": annotation, "sender": "system"}
    )
    await manager.broadcast_to_process(message, process_id)


async def broadcast_simulation_result(
    simulation_data: dict, process_id: str = "default"
):
    """
